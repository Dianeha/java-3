...
// 이벤트 프로그램 작성하는 방벙
// 1. 컴퍼넌트(컨테이너)를 이용해 프레임 생성 : 디자인 클래스
// => 컴퍼넌트(컨테이너)에서는 다양한 이벤트 발생 -이벤트 소스
// 2. 이벤트 소스에서 이벤트가 발생될 경우 이벤트 처리를 목적으로 하는 클래스- 이벤트 핸들러
// => 발생된 이벤트 처리를 목적으로 하는 리스너 인터페이스를 상속받아 작성
// => XXXEvent 클래스로 생성된 이벤트를 처리하기 위해 XXXListener 인터페이스를 제공 
//    ex) ActionEvent 클래스의 이벤트 발생 > ActionListener 인터페이스 상속받아 작성
// => 리스너 인터페이스의 추상메소드를 오버라이드 선언해 이벤트 처리 명령
// 3. 이벤트 소스에서 이벤트가 발생될 경우 이벤트 핸들러의 메소드 호출
...
public class EventHandlerApp extends Frame {
	...
	public EventHandlerApp(String title) {
		...
		//ActionListener 인터페이스를 상속받은 자식클래스의 인스턴스를 전달하여 버튼에서
		//발생될 이벤트를 처리하도록 이벤트 핸들러 등록
		// => 이벤트가 발생될 경우 자동으로 등록된 이벤트 핸들러의 메소들가 호출
...
class ExitButtonEventHandler implements ActionListener { //인터페이스 상속 - 추상메소드 오버라이드
	
	//Listener 인터페이스의 추상메소드를 오버라이드 선언 - 이벤트 처리 목적
	// => 해당 이벤트가 발생될 경우 자동 호출
	@Override
	public void actionPerformed(ActionEvent e) {
		//System.exit(int status); : 프로그램에 존재하는 모든 스레드를 제거
		// => 프로그램 종료 메소드
		System.exit(0);
	}
}