...
public class NotepadApp extends JFrame implements ActionListener {
	...
	public NotepadApp(String title) {
		...
		// JTextArea 컴퍼넌트에서 키보드 관련 이벤트가 발생된 경우 처리될 이벤트 핸들러 등록(익명)
		area.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
				// 이벤트를 발생한 키보드 문자값이 기능키가 아닌 경우 필드값 변경
				if (e.getKeyChar() != '\0')
					state = true;
			}
		});
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (eventSource == init) {
			if (saveConfirm()) {
				...
				// JFrame 컴퍼넌트의 제목 초기화
				setTitle("제목 없음 - 메모장");
				// 편집중인 파일경로를 저장하는 필드 초기화
				filePath = null;
				// 변경유무 상태정보를 저장하는 필드 초기화
				state = false;
			}
			...
			// => FileDialog 컴퍼넌트에서 파일을 선택하거나 취소한 경우
	...
	// JTextArea 컴퍼넌트의 내용이 변경된 경우 특정이벤트가 발생한 경우
	// JTextArea 컴퍼넌트 변경 내용 저장 유무를 선택처리하기 위한 메소드
	// => 반환값 -false : 취소선택, true : 저장 또는 미저장
	private boolean saveConfirm() {
		if (state) {// JTextArea 컴퍼넌트의 내용이 변경된 경우
			// ■ JOptionPane.showConfirmDialog(parentComponent,String message,String title,
			// Int optionType)
			// => 선택다이얼로그를 출력하는 메소드 >> 선택 결과값 반환
