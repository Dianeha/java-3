...
public class ConnectServerApp {
	public static void main(String[] args) throws IOException {
		...
		// => 포트번호를 전달하여 해당 네트워크를 열어 클라이언트의 접속을 허용
		...
		try {
			...
			while (true) {
				...
				// => 클라이언트 접속 전까지 스레드 일시중지
				// => 클라이언트가 접속될 경우 클라이언트의 네트워크 정보와 입출력 스트림 정보를 저장하고 있는 소켓 인스턴스 반환
				Socket socket = server.accept();
				
				System.out.println("[메세지]클라이언트("+socket.getInetAddress().getHostAddress()+")에서 접속하셨습니다.");
